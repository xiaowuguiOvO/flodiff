...
#         self.trajs_dir = os.path.join(self.data_folder, self.scene_name)
...
#             begin = 3
#             for time in range(begin,traj_len - 11):
#                 self.index_to_data.append((traj_name, time))
...
#         f_curr , curr_time = self.index_to_data[index]
#          # Load images
#         context = []
...
#         curr_pos = self._get_trajectory(f_curr)[curr_time][:2]

#         #load floorplan
#         floorplan = self._load_image(self.scene_name, "floorplan")
#         return  (
#             torch.as_tensor(obs_image, dtype=torch.float32), 
#             torch.as_tensor(curr_pos, dtype=torch.float32), 
#             torch.as_tensor(floorplan, dtype=torch.float32)
#         )
...
# need to prune
# ========================
class position_dataset(Dataset):
    def __init__(
        self,
        data_folder: str,
        scene_name: str,
    ...
    def _get_pos_ori(self, traj_data, curr_time):
        start_index = curr_time
        cur_pos = traj_data[start_index, :2].copy()
        cur_ori = traj_data[start_index, 2:].copy()
        cur_ori = cur_pos + (cur_ori - cur_pos) / np.linalg.norm(cur_ori - cur_pos)
        goal_pos = traj_data[-1 - self.end_slack, :2].copy()

        
        if self.normalize:
            cur_pos /= self.metric_waypoint_spacing * self.waypoint_spacing  # transform meters to waypoints(steps)
            cur_ori /= self.metric_waypoint_spacing * self.waypoint_spacing
            goal_pos /= self.metric_waypoint_spacing * self.waypoint_spacing
        return  cur_pos, cur_ori, goal_pos
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        ...
        cur_pos, cur_ori, goal_pos = self._get_pos_ori(curr_traj_data, curr_time)   # waypoints(steps) metric in local
