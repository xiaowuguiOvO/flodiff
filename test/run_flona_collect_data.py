...
# scene_names = ['Mosquito_1', 'Roane_0', 'Stilwell_0']

def main(headless=False, num_episodes=10, num_steps=200, scene_config_path=None, model_config_path=None):
    
    for cur_scene_name in scene_names:
        print(f"==============================================")
        print(f"========== Running Scene: {cur_scene_name} ==========")
        print(f"==============================================")
        
        last_underscore_index = cur_scene_name.rfind('_')
        scene_id = cur_scene_name[:last_underscore_index]
        scene_floor_num = int(cur_scene_name[last_underscore_index + 1:])
        ...
        scene_config["floor_num"] = scene_floor_num
        
        os.makedirs(data_save_dir, exist_ok=True)
        env_mode = "headless" if headless else "gui_interactive"
        env = iGibsonEnv(
            config_file=scene_config,
            mode=env_mode,
            use_pb_gui=True
        )
        ...
        # load traj and get goal point,start position
        scene_dir = os.path.join(test_dir, scene_name)
        traj_names = sorted([
        d for d in os.listdir(scene_dir)
        if d.startswith('traj_') and os.path.isdir(os.path.join(scene_dir, d))
        ])
        
        for traj_name in traj_names:
            
            print(f"####### running scene{scene_name}, {traj_name} #######")
            traj_data_path = os.path.join(scene_dir, traj_name, f"{traj_name}.npy")
            traj_data = np.load(traj_data_path)
            
            traj_robot_init_pos = traj_data[0, :2]
            traj_goal_pos = traj_data[-2, :2]
            ...
            for episode in range(num_episodes):
                ...
                # get floor z
                igbson_path = 'iGibson/igibson/data/g_dataset'
                floor_data_path = os.path.join(igbson_path, scene_id, 'floors.txt')
                floor_data = []
                ...
                for step in range(num_steps):
                    ...
                    if next_goal_point is not None and will_collision_flag != 1:
                        ...
                        look_at_idx = next_goal_point_idx + 6
                    ...
                    if IS_DECISION_FLAG:
                        ...
                        #     time_step=step,
                        #     save_path=os.path.join(viz_dir, f"{scene_name}, {traj_name},step_{step}.png"),
                        #     show_obs=True,
...
if __name__ == "__main__":
    ...
    main(headless=run_headless, num_episodes=1, num_steps=2000, scene_config_path=scene_config_path, model_config_path=model_config_path)
